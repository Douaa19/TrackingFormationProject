        ...
        <a href="{{route('admin.dashboard')}}" class="logo logo-dark">
            ...
            <span class="logo-lg">
                <img src="{{ getImageUrl(getFilePaths()['site_logo']['path'] . "/" . site_settings('site_logo_lg')) }}"
                    alt="{{site_settings('site_logo_lg')}}" height="35">
            </span>
        </a>
        ...
        <div class="container-fluid">
            ...
            <ul class="navbar-nav gap-1" id="navbar-nav">
                ...
                @if(site_settings('chat_module') == App\Enums\StatusEnum::true->status())
                    <li class="nav-item">
                        <a class="nav-link menu-link {{url()->current() === url('admin/tickets/testmsg') ? 'active' : ''}}"
                            href="{{ url('admin/tickets/testmsg') }}">
                ...
                @if(check_agent('system_configuration') || check_agent('manage_tickets') || check_agent('manage_category') || check_agent('manage_priorites'))
                    ...
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ !request()->routeIs('admin.setting.ticket.*') ? "collapsed" : ""  }}  "
                            href="#ticketConfiguration" data-bs-toggle="collapse" role="button" aria-expanded="false"
                            aria-controls="ticketConfiguration">
                            ...
                            <ul class="nav nav-sm flex-column gap-1">
                                <li class="nav-item">
                                    <a href="{{route('admin.setting.ticket.configuration')}}"
                                        class="nav-link {{ request()->routeIs('admin.setting.ticket.configuration') ? 'active' : ""  }}  ">
                                        {{translate('General Configuration')}}
                                    </a>
                                </li>

                                <li class="nav-item">
                                    <a href="{{route('admin.trigger.list')}}"
                                        class="nav-link {{ request()->routeIs('admin.trigger.*') ? 'active' : ""  }}  ">
                ...
                @if(auth_user()->agent == App\Enums\StatusEnum::false->status())
                    ...
                    <li class="nav-item">
                        <a class="nav-link menu-link {{   !request()->routeIs('admin.agent.*') || !request()->routeIs('admin.group.*')  }}  "
                            href="#manageAgent" data-bs-toggle="collapse" role="button" aria-expanded="false"
                        ...
                        <div class="pt-1 collapse {{ request()->routeIs('admin.agent.*') || request()->routeIs('admin.group.*') ? "show" : ""  }}  menu-dropdown"
                            id="manageAgent">
                            <ul class="nav nav-sm flex-column gap-1">
                ...
                @if(check_agent('manage_users'))
                    ...
                    <li class="nav-item">
                        <a class="nav-link menu-link {{   !request()->routeIs('admin.user.*')  }}  " href="#manageUser"
                            data-bs-toggle="collapse" role="button" aria-expanded="false" aria-controls="manageUser">
                            <i class="ri-team-line"></i><span>
                                {{translate('Manage User')}}
                            </span>
                        </a>

                        <div class="pt-1 collapse {{ request()->routeIs('admin.user.*') ? "show" : ""  }}  menu-dropdown"
                            id="manageUser">
                            <ul class="nav nav-sm flex-column gap-1">
                ...
                @if(check_agent('manage_frontends') || check_agent('manage_pages') || check_agent('manage_faqs'))
                    ...
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ (!request()->routeIs('admin.frontend.*') || !request()->routeIs('admin.menu.*') || !request()->routeIs('admin.page.*') || !request()->routeIs('admin.faq.*')) ? "collapsed" : ""  }}  "
                            href="#manageFrontend" data-bs-toggle="collapse" role="button" aria-expanded="false"
                            aria-controls="manageFrontend">
                            <i class="ri-earth-line"></i> <span>
                                {{translate('Appearance Settings')}}
                            </span>
                        </a>
                        <div class="pt-1 collapse {{ request()->routeIs('admin.frontend.*') || request()->routeIs('admin.menu.*') || request()->routeIs('admin.faq.*') || request()->routeIs('admin.page.*') ? "show" : ""  }}  menu-dropdown"
                            id="manageFrontend">
                            <ul class="nav nav-sm flex-column gap-1">
                ...
                @if(check_agent('manage_frontends'))
                    <li class="nav-item">
                        ...
                        <a class="nav-link menu-link {{ (!request()->routeIs('admin.article.*') || !request()->routeIs('admin.category.article.create') || !request()->routeIs('admin.category.article.show')) ? "collapsed" : ""  }}  "
                            href="#manageArticle" data-bs-toggle="collapse" role="button" aria-expanded="false"
                            aria-controls="manageArticle">
                            <i class="ri-article-line"></i> <span>
                                {{translate('Article Administration')}}
                            </span>
                        </a>
                        <div class="pt-1 collapse {{ request()->routeIs('admin.article.*') || request()->routeIs('admin.category.article.create') || request()->routeIs('admin.category.article.show') ? "show" : ""  }}  menu-dropdown"
                            id="manageArticle">
                            <ul class="nav nav-sm flex-column gap-1">
                ...
                @if(check_agent('manage_frontends'))
                    ...
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ (!request()->routeIs('admin.contact.subscriber') || !request()->routeIs('admin.contact.list')) ? "collapsed" : ""  }}  "
                            href="#promotional" data-bs-toggle="collapse" role="button" aria-expanded="false"
                            aria-controls="promotional">
                            <i class="ri-funds-line"></i> <span>
                                {{translate('Marketing/Promotion')}}
                            </span>
                        </a>
                        <div class="pt-1 collapse {{ request()->routeIs('admin.contact.subscriber') || request()->routeIs('admin.contact.list') ? "show" : ""  }}  menu-dropdown"
                            id="promotional">
                            <ul class="nav nav-sm flex-column gap-1">
                ...
                @if(check_agent('mail_configuration'))
                    <li class="nav-item">
                        <a class="nav-link  {{!request()->routeIs('admin.mail.*') ? "collapsed" : ""   }}   menu-link"
                            href="#emailConfiguration" data-bs-toggle="collapse" role="button" aria-expanded="false"
                            aria-controls="emailConfiguration">
                        ...
                        <div class="collapse {{request()->routeIs('admin.mail.*') ? "show" : ""   }}
                            ...
                            <ul class="nav nav-sm flex-column gap-1">
                                ...
                                <li class="nav-item">
                                    <a href="{{route('admin.mail.incoming')}}"
                                        class='{{request()->routeIs('admin.mail.incoming') ? 'active' : ''}}  nav-link'>
                ...
                @if(check_agent('sms_configuration'))
                    ...
                    <li class="nav-item">
                        <a class="nav-link menu-link {{ !request()->routeIs('admin.sms.*') ? "collapsed" : ""  }}  "
                            href="#smsConfiguration" data-bs-toggle="collapse" role="button" aria-expanded="false"
                            aria-controls="smsConfiguration">
                            ...
                            <ul class="nav nav-sm flex-column gap-1">
                                <li class="nav-item">
                                    <a href="{{route('admin.sms.gateway.index')}}"
                                        class="nav-link {{ request()->routeIs('admin.sms.gateway.index') || request()->routeIs('admin.sms.gateway.edit') ? 'active' : ""  }}  ">
                                ...
                                <li class="nav-item">
                                    <a href="{{route('admin.sms.template.index')}}"
                                        class=" {{ request()->routeIs('admin.sms.template.index') || request()->routeIs('admin.sms.template.edit') ? 'active' : ""  }}  nav-link">
